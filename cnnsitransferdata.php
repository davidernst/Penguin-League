...
for ($whichteam=0; $whichteam<$numofteams; $whichteam++)
{
	...
	$theurl = "http://sportsillustrated.cnn.com/baseball/mlb/teams/stats/2003/".$teams[$whichteam]."/0_batting.html";
	...
	*/
	...
	do {
		...
		if ((ereg("TData2", $player) == TRUE) OR (ereg("TData1", $player) == TRUE))
		{
			$player = fgets($fp, 1024);
			$stat2 = split("/", $player);
			$the_id = $stat2[6];
	...
	} while (ereg("STATS, Inc.", $player) == FALSE);

	fclose($fp);

	// Pitchers============================

	$theurl = "http://sportsillustrated.cnn.com/baseball/mlb/teams/stats/2003/".$teams[$whichteam]."/0_pitching.html";
	...
	do {
		...
		if ((ereg("TData2", $player) == TRUE) OR (ereg("TData1", $player) == TRUE))
		{
			...
			mysql_query ("INSERT INTO dick_league_pitchers (id, team, full_name, pos, games, era, win, loss, save, inning, h, bb, k, hr, r, er, start, complete, shutout) VALUES('$the_id', '$abrvs[$whichteam]', '$fullname', 'P', '$parsed2[5]', '$parsed2[0]', '$parsed2[1]', '$parsed2[2]', '$parsed2[3]', '$parsed2[9]', '$parsed2[10]', '$parsed2[14]', '$parsed2[15]', '$parsed2[13]', '$parsed2[11]', '$parsed2[12]', '$parsed2[6]', '$parsed2[7]', '$parsed2[8]')");
		}
	} while (ereg("STATS, Inc.", $player) == FALSE);
